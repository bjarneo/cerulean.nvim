-- Cerulean theme colors and highlight definitions
local M = {}
...
M.colors = {
	...
	-- Strings
	string = "#FFF9AF",
	string_template = "#FFF9AF",
	string_escape = "#FFF9AF",
	string_regex = "#FFF9AF",
...
function M.apply(config)
	...
	if config.plugins.treesitter then
		require("cerulean.plugins.treesitter").apply(colors, config)
	end

	if config.plugins.lsp then
		require("cerulean.plugins.lsp").apply(colors, config)
	end

	if config.plugins.telescope then
		require("cerulean.plugins.telescope").apply(colors, config)
	end

	if config.plugins.nvimtree then
		require("cerulean.plugins.nvimtree").apply(colors, config)
	end

	if config.plugins.whichkey then
		require("cerulean.plugins.whichkey").apply(colors, config)
	end

	if config.plugins.gitsigns then
		require("cerulean.plugins.gitsigns").apply(colors, config)
	end

	if config.plugins.indent_blankline then
		require("cerulean.plugins.indent_blankline").apply(colors, config)
	end

	if config.plugins.markdown then
		require("cerulean.plugins.markdown").apply(colors, config)
	end

	if config.plugins.hlchunk then
		require("cerulean.plugins.hlchunk").apply(colors, config)
	end
